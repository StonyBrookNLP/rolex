...
from warmup import WarmupLinearScheduler
...
class NL2LTLDataSetClass(Dataset):
    """
...
def T5Trainer(
    ...
    training_set = NL2LTLDataSetClass(
        train_dataset,
        tokenizer,
    ...
    scheduler = WarmupLinearScheduler(optimizer,warmup_steps=2000,scheduler_steps=10000,min_ratio=0,fixed_lr=False)
...
if __name__ == "__main__":
    ...
    #For training with weak targets
    new_input = "NL2CODE\nNL:"+df["nl"]+"\nPK:"+df["pk_generator"]
    target = df["weak_target"]
    
    new_df = pd.DataFrame({"input":new_input,"target":target})

    #For validating with weak targets
    new_input = "NL2CODE\nNL:"+val_df["nl"]+"\nPK:"+val_df["pk_generator"]
    target = val_df["weak_target"]
