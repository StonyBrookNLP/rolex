...
class NL2CMDDataSetClass(Dataset):
    """
...
def T5Trainer(
    ...
    training_set = NL2CMDDataSetClass(
        train_dataset,
        tokenizer,
...
if __name__ == "__main__":
    ...
    #Input df transformation
    new_input = "NL2CMD\nNL:"+df["nl"]+"\nPK:"+df["pk_retrieved"]
    target = df["weak_target"]+"<SEP>"+df["fl"]
    
    new_df = pd.DataFrame({"input":new_input,"target":target})

    #Val df transformation
    new_input = "NL2CMD\nNL:"+val_df["nl"]+"\nPK:"+val_df["pk_retrieved"]
    target = val_df["weak_target"]+"<SEP>"+val_df["fl"]
